...
func finishNetwork(cfgPath string, result *Result, fromCaptured, toCaptured string, addedSysmon []map[string]any) {
	mergeSysmonDNS(result, fromCaptured, toCaptured, addedSysmon)
	attachSnapshotHTTP(cfgPath, result, fromCaptured, toCaptured)
	if result.Network != nil {
		result.Network.DNS = FilterDNSBySnapshotWindow(result.Network.DNS, fromCaptured, toCaptured)
		result.Network.DNS = DedupDNSByQuery(result.Network.DNS)
