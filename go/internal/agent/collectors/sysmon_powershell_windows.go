...
$out = foreach ($r in $records) {
  ...
  [ordered]@{
    eid = [int]$r.Id
    t = switch ([int]$r.Id) { 1 {'ProcessCreate'} 11 {'FileCreate'} 12 {'FileCreateStream'} 13 {'RegistryEvent'} 22 {'DnsQuery'} 23 {'FileDelete'} 26 {'FileDeleteDetected'} default {"Event$($r.Id)"} }
